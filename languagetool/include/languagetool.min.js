...
// Declare Variables
 ...
 var lt_viewportProcess = lt_set['lt_viewport_process'] || 'disabled';
...
if ( lt_viewportProcess != 'disabled' && i == 1 && last_winPos == winPos) {
    continue;
}

        var tarray = [];
        for ( var i2 in sgstore['queue'][i] ) {
    ...
    if ( lt_viewportProcess == 'only' && sgcheck[ key ].length >= 2 && lt_onscreen( $(sgstore['obj'][key]) ) != 1 ) {
lastActive=0;
        tarray.push(key);
       continue;
    } else if ( lt_viewportProcess == 'nearby' && sgcheck[ key ].length >= 2 && lt_onscreen( $(sgstore['obj'][key]) ) == 0 ) {
lastActive=0;
        tarray.push(key);
       continue;
    }
