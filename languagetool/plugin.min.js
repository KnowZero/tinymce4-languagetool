...
var statusbar = editor.theme.panel && editor.theme.panel.find('#statusbar')[0];
 ...
 var sgstore = { html:{}, text:{}, obj:{}, lt:{}, lock:{}, run:[], contextmenu:{}, ignore:{ once:{}, all:{}, rule:{}}, queue:[[],[],[]], progress:[0,0,0,0], issues:{}  }; // Universal Storage
 ...
 var lt_maxInstances = (editor.getParam('lt_max_instances') || 1)-1;
...
var buttonArgs = {
 ...
 var timerFunc = function() {
    ...
    if ( timerRunner >= lt_remoteTimer ) {
        ...
        var tsize=0;
        var newInstance=getUUID();
        listInstances[newInstance]={ text:'', size:0 };
       ...
       sgstore['queue'][i] = $.grep(sgstore['queue'][i], function(key,i) {
            ...
            if (  key in sgcheck  ) {
                ...
                var val=sgcheck[ key ];
                    ...
                    if ( listInstances[newInstance]['text'] == '' || (listInstances[newInstance]['text'].length+tval.length) < lt_sizeOfInstances ) {
                        listInstances[newInstance]['text']=listInstances[newInstance]['text']+tval;
                        tsize++;  
...
//console.log( Object.keys(sgcheck).length + '='+sgstore['queue'][0].length + '='+sgstore['queue'][1].length );
        ...
        var tkey = newInstance;

        data['text']=listInstances[ tkey ]['text'];
        ...
        if ( data['text'] != '' ) {
            ...
            $.ajax({
                ...
                url: lt_remoteUrl,
                data: data,
                indexValue: { text: data['text'], tsize: tsize  },
 ...
 editor.on('BeforePastePreProcess', function (e) {
    sgstore['progress'][3]=Math.round(e.content.length / 500);
    lt_progress_bar(0, sgstore['pastesize']  );
...
//editor.execCommand('mceReplaceContent',false,$(this).data("ident-value") );
 ...
 function lt_clearHighlighter (content) {
    ...
    var temphtml= content.replace(/<span class="lt-highlight hl[rgnybopt][0-9]" data-hid="[0-9]+" data-word="[a-zA-Z0-9\/=+]+" data-rule="[a-zA-Z0-9_]+">(.*?)<\/span>/g, function(match,p1) { return p1; });
...
//lt_debug("mode="+mode);
 ...
 function lt_openContextMenu (e) {
    ...
    while ( nfind != 1 ) {
        if ( mynode.nodeName == 'STRONG' || mynode.nodeName == 'EM' || mynode.nodeName == 'SUP' || mynode.nodeName == 'SUB' || (mynode.nodeName == 'SPAN' && /hl[rgnybopt][0-9]/.test($(mynode).attr('class')) == false ) ) {
...
$(progress_bar).val( sgstore['progress'][0] );
    ...
    $(progress_issues).html( $(tinymce.activeEditor.getBody()).find('.lt-highlight').length + ' Issue(s)'  );
            ...
            editor.windowManager.open({
                ...
                body: [
                    { type: 'label', html: '?', id: 'lt_issueMessage' },
                   {
                    ...
                    label  : '-    Action:',
                    items  : [
                        { text: 'Apply', value: 'Apply', onClick: function() { lt_contextMenuClick( $('#lt_issueSuggestions').data('ident-UUID'), $('#lt_issueSuggestions').data('ident-HID'), $('#lt_issueSuggestions').val(), 1 );  } },
                    ...
                    { 
type: 'selectbox', size: 10, name: 'errors', id: 'lt_issueSuggestions', 
size: 10, options: [], 
...
function lt_nextIssue () {

    var mynode = $(tinymce.activeEditor.getBody()).find('.lt-highlight').not('.skip-dialog-highlight').get(0);
...
$('#lt_issueMessage').text( ( (sgstore['lt'][uuid][hid]['shortMessage'] == '' || lt_fullMessage == true ) ? sgstore['lt'][uuid][hid]['message']:sgstore['lt'][uuid][hid]['shortMessage']) );
...
function lt_contextMenuClick(uuid,hid,value,mode) {
    ...
    $(progressbar).html( $(tinymce.activeEditor.getBody()).find('.lt-highlight').length + ' Issue(s)'  );
